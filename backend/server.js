...
app.get('/api/spaces', async (req, res) => {
  try {
    ...
    const formatted = spaces.map(space => ({
      ...
      photo_urls: photos.filter(p => p.space_id === space.id).map(p => `/uploads/${p.file_path}`),
      cae_codes: caeCodes.filter(c => c.space_id === space.id).map(c => ({ code: c.code, description: c.description }))
    }));
...
app.get('/api/spaces/:id', async (req, res) => {
  try {
    ...
    if (spaces.length === 0) return res.status(404).json({ error: 'Espaço não encontrado' });
    ...
    const formatted = {
      ...
      cae_codes: caeCodes.map(c => ({ code: c.code, description: c.description }))
    };
...
app.post('/api/spaces', upload.array('images', 5), async (req, res) => {
  try {
    ...
    if (!req.session.user) return res.status(401).json({ error: 'Necessário estar logado para criar um espaço.' });
...
app.get('/api/cae-codes', async (req, res) => {
  try {
    const [rows] = await db.query('SELECT id, code, description FROM cae_codes');
    res.json(rows);
  } catch (err) {
...
app.put('/api/spaces/:id', upload.array('images', 5), async (req, res) => {
  try {
    ...
    if (!req.session.user) return res.status(401).json({ error: 'Necessário estar logado para editar um espaço.' });
    ...
    // Atualiza CAEs (apaga os antigos e insere os novos)
    if (caes) {
      const caeIds = JSON.parse(caes);
      await db.query('DELETE FROM space_cae WHERE space_id = ?', [spaceId]);
      for (const caeId of caeIds) {
        await db.query(`INSERT INTO space_cae (space_id, cae_id) VALUES (?, ?)`, [spaceId, caeId]);
      }
    }
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (results.length === 0) return res.status(401).json({ message: 'Utilizador não encontrado.' });
    ...
    if (passwordMatch) {
      ...
      res.json({ message: 'Login realizado com sucesso!', name: user.name, role: user.role });
    } else {
...
io.on('connection', socket => {
  ...
  socket.on('send_message', async data => {
    ...
    try {
      await db.query('INSERT INTO messages (sender_id, receiver_id, content) VALUES (?, ?, ?)', [sender_id, receiver_id, content]);
    } catch (err) {
...
app.get('/api/blocked-dates/:spaceId', async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT * FROM blocked_dates WHERE space_id = ? ORDER BY start_datetime',
      [req.params.spaceId]
    );
    res.json(rows);
  } catch (err) {
...
app.post('/api/blocked-dates', requireRole(['admin', 'owner']), async (req, res) => {
  try {
    ...
    if (!space_id || !start_datetime || !end_datetime) return res.status(400).json({ error: 'Campos obrigatórios em falta.' });
    ...
    res.status(201).json({ message: 'Data bloqueada adicionada com sucesso!', id: result.insertId });
  } catch (err) {
...
app.post('/api/blocked-dates/batch', requireRole(['admin','owner']), async (req, res) => {
  try {
    ...
    if (!space_id || !Array.isArray(dates)) return res.status(400).json({ error: 'Formato inválido.' });
...
app.delete('/api/blocked-dates/:id', requireRole(['admin', 'owner']), async (req, res) => {
  try {
    ...
    if (dates.length === 0) return res.status(404).json({ error: 'Data bloqueada não encontrada.' });

    if (currentUserRole !== 'admin' && dates[0].created_by !== currentUserId)
